...
- **`tvpred()`** – Performs **multi-step ahead forecasting** under the estimated model, optionally incorporating parameter and factor uncertainty.  
...
Performs **penalized estimation** of a time-varying VAR (TV-VAR) model using the **ECM algorithm** with proximal gradient descent.  
The function introduces an L1 (lasso-type) penalty on the factor loadings to encourage sparsity. It supports both regular Lasso and adaptive Lasso.
...
Unlike standard VAR models, where IRFs are constant, `tvirf()` produces a **sequence of IRFs across time**, reflecting how dynamic relationships change throughout the sample.  
This is particularly useful for analyzing how the transmission of shocks evolves during different economic or financial regimes.
...
After estimating a model with `tvfit()` or `tvpenfit()`, you can use the fitted object to generate forecasts of future observations. The function `tvpred()` simulates predictive distributions by combining the estimated time-varying parameters with Monte Carlo draws of future shocks. For example, using the unpenalized fit from earlier:
